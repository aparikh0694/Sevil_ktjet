...
ClassImp(ktHiFastJet)

ktHiFastJet::ktHiFastJet()
{
...
ktHiFastJet::ktHiFastJet(Int_t nXiBins,Double_t xiMax)
{
...
void ktHiFastJet::Init()
{
  // Dummy !!!
}

void ktHiFastJet::PrintInfo()
{
  cout<<endl;
...
void ktHiFastJet::SetFiducial(Double_t mEta,Double_t mPhi)
{
  mEtaMax=mEta;
  mPhiMax=mPhi;
}

void ktHiFastJet::SetAreaDefinition(Double_t m_ghost_etamax, Int_t m_active_area_repeats, Double_t m_ghost_area)
{
...
void ktHiFastJet::PrintAreaDefinition()
{
  cout<<endl;
...
void ktHiFastJet::AddParticle(Double_t px, Double_t py, Double_t pz, Double_t E)
{
...
void ktHiFastJet::AddParticle(Double_t px, Double_t py, Double_t pz, Double_t E, Bool_t charged)
{

  // DEBUG:
  //cout<<" ---> In : ktHiFastJet::AddParticle(Double_t px, Double_t py, Double_t pz, Double_t E, Bool_t charged)"<<endl;
...
void ktHiFastJet::AddParticle(Double_t px, Double_t py, Double_t pz, Double_t E, Bool_t charged, Int_t sign)
{
...
void ktHiFastJet::AddParticle(Double_t px, Double_t py, Double_t pz, Double_t E,Bool_t charged,ktPID* mPid)
{
  // just for backward comp.
...
void ktHiFastJet::AddParticle(Double_t px, Double_t py, Double_t pz, Double_t E,ktPID* mPid)
{

  // DEBUG:
  //cout<<" In:  ktHiFastJet::AddParticle(Double_t px, Double_t py, Double_t pz, Double_t E, Bool_t charged, ktPID* mPid)"<<endl;
...
void ktHiFastJet::AddQuenchedJet(ktJetQuench *mQ)
{
  ...
  if (mQ!=0)
    ...
    cout<<" ---> Warning ktJetQuench zero pointer in ktHiFastJet !!!'"<<endl;

}

void ktHiFastJet::AddUnQuenchedJet(ktJetQuench *mQ)
{
...
void ktHiFastJet::AddPythia8Event(ktPythia8 *myp,TString mSel)
{
...
Double_t ktHiFastJet::GetdPhi(Double_t mphi,Double_t vphi)
{
...
Bool_t ktHiFastJet:: IsTriggerJet(ktMuFastJet *mJ,ktMuEvent *ev)
{
...
void ktHiFastJet::GetJetFF(ktMuFastJet *mJ,Bool_t setInJet, Bool_t mSave)
{
...
void ktHiFastJet::FillFF(TString mAlgo, ktMuEvent *ev) //, Int_t nJet)
{
  if (!ev==0 && fillFF)
    {
      if (info)
...
void ktHiFastJet::DoXiBkg(TString mAlgo,ktMuEvent *ev)
{
...
void ktHiFastJet::DoPhiBkg(TString mAlgo,ktMuEvent *ev)
{
...
void ktHiFastJet::RunSISCone(Double_t Rc,ktMuEvent *ev)
{
  ...
  // see manual and use passive or direct from SISCone !???
  ...
  if (input_particles.size()>0)
    ...
    cout<<"  ---> Warning (ktHiFastJet) : Empty event !!!"<<endl;
  
  delete plugin;

}

void ktHiFastJet::DoBkg(Double_t Rparam,TString mAlgo)
{
  ...
  fastjet::ActiveAreaSpec area_spec_bkg(ghost_etamax, active_area_repeats, 
				    ghost_area);
 ...
 if (input_particles.size()>0)
    {
      fastjet::ClusterSequenceArea clust_seq(input_particles, jet_def_bkg, area_spec_bkg);
      fastjet::Selector selector = fastjet::SelectorAbsRapMax(5) * (!fastjet::SelectorNHardest(2));

      fastjet::JetMedianBackgroundEstimator bkg_estimator(selector, jet_def_bkg, area_spec_bkg);      
    ...
    cout<<"  ---> Warning (ktHiFastJet::DoBkg) : Empty event !!!"<<endl;
...
void ktHiFastJet::RunFastJetSub(Double_t Rparam,TString mAlgo,ktMuEvent *ev)
{
...
Bool_t ktHiFastJet::GetHighestJetForQuenching(ktJetQuench *mQ,Double_t Rparam,TString mAlgo)
{
  ...
  if (input_particles.size()>0 && mQ!=0)
	...
	cout<<"  ---> Warning (ktHiFastJet) : No jet for quenching found !!!"<<endl; 
